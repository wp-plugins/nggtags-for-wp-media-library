// extracted from .../wp-includes/js/media-models.js
...
(function($){
	...
	Attachment = media.model.Attachment = Backbone.Model.extend({
		...
		saveCompat: function( data, options ) {
			...
			var ret=media.post( 'save-attachment-compat', _.defaults({
				id:      this.id,
				nonce:   this.get('nonces').update,
				post_id: 0
            ...
            if(media.nggml.nggmlScreenOptions.use_alt_media_list_pane){
                jQuery("div#updating-overlay-for-table-media-row").css("display","block");
            }else{
                var tr=jQuery("tbody#the-list tr#post-"+model.id);
                media.nggml.createUpdatingOverlayForTableMediaRow(tr,true);
            }
            return ret;
		},
        refreshMediaTableRow:function(){
            var id=this.id;
            r={action:"nggml_get_media_table_rows",data:[id]};
            jQuery.post(ajaxurl,r,function(r){
                var tr=jQuery("tbody#the-list tr#post-"+id);
                var h=tr.height();
                var d=tr.find("td.title div.row-actions").css("display");
                tr.replaceWith(r);
                tr=jQuery("tbody#the-list tr#post-"+id);
                tr.find("td.title div.row-actions").css("display",d);
                tr.height(h);
                if(media.nggml.nggmlScreenOptions.use_alt_media_list_pane){
                    jQuery("tr#nggml-alt-media-list div#nggml-alt-media-list-titles li#nggml-li-"+id+" span.nggml-image-title-text")
                        .text(tr.find("td.title a").first().text());
                    jQuery("tr#nggml-alt-media-list div#nggml-alt-media-list-images div#nggml-div-"+id+" img")
                        .prop("src",tr.find("td.media-icon img").prop("src"));
                    jQuery("div#updating-overlay-for-table-media-row").css("display","none");
                }else{
                    jQuery("div#updating-overlay-for-table-media-row").remove();
                }
                tr.find("td.title div.row-actions span.edit a").click(function(e){
                ...
                tr.find("td.title div.row-actions span.delete a").click(function(e){
                    var id=jQuery(this).parents("tr")[0].id.substr(5);
                    e.stopImmediatePropagation();
                    e.stopPropagation();
                    e.preventDefault();
	...
	Attachments = media.model.Attachments = Backbone.Collection.extend({
        ...
        remove:function removeBc(){
            return media.model.Attachments.prototype.__proto__.remove.apply(this,arguments);
        },
	...
	media.query = function( props ) {
		var attachments = new Attachments( null, {
